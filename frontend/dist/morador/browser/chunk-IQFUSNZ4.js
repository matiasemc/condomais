...
// libs/core/src/lib/state/auth.state.ts
var AuthState = class _AuthState {
  constructor() {
    ...
    this.memberships = signal([], ...ngDevMode ? [{ debugName: "memberships" }] : (
      /* istanbul ignore next */
      []
    ));
    this.currentTenant = signal(null, ...ngDevMode ? [{ debugName: "currentTenant" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.currentRole = computed(() => {
      const tenant = this.currentTenant();
      if (!tenant)
        return null;
      return this.memberships().find((m) => m.tenant.id === tenant.id && m.ativo)?.role ?? null;
    }, ...ngDevMode ? [{ debugName: "currentRole" }] : (
      /* istanbul ignore next */
      []
    ));
    this.currentUser = computed(() => this.user(), ...ngDevMode ? [{ debugName: "currentUser" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.activeMemberships = computed(() => this.memberships().filter((m) => m.ativo), ...ngDevMode ? [{ debugName: "activeMemberships" }] : (
      /* istanbul ignore next */
      []
    ));
    this.hasMultipleTenants = computed(() => this.activeMemberships().length > 1, ...ngDevMode ? [{ debugName: "hasMultipleTenants" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isMasterAdmin = computed(() => this.profile()?.isMasterAdmin ?? false, ...ngDevMode ? [{ debugName: "isMasterAdmin" }] : (
      /* istanbul ignore next */
      []
    ));
  }
  ...
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _AuthState, factory: _AuthState.\u0275fac, providedIn: "root" });
  }
};
(() => {
...
var PostgrestClient = class PostgrestClient2 {
  ...
  */
  constructor(url, { headers = {}, schema, fetch: fetch$1, timeout, urlLengthLimit = 8e3, retry } = {}) {
    ...
    if (timeout !== void 0 && timeout > 0) this.fetch = (input2, init) => {
      ...
      if (existingSignal) {
        ...
        return originalFetch(input2, _objectSpread2(_objectSpread2({}, init), {}, { signal: controller.signal })).finally(() => {
          clearTimeout(timeoutId);
          existingSignal.removeEventListener("abort", abortHandler);
        });
      }
      return originalFetch(input2, _objectSpread2(_objectSpread2({}, init), {}, { signal: controller.signal })).finally(() => clearTimeout(timeoutId));
    };
...
var WebAuthnApi = class {
   ...
   */
  _challenge(_0, _1) {
    return __async(this, arguments, function* ({ factorId, webauthn, friendlyName, signal: signal2 }, overrides) {
      var _a;
      try {
   ...
   */
  _authenticate(_0, _1) {
    return __async(this, arguments, function* ({ factorId, webauthn: { rpId = typeof window !== "undefined" ? window.location.hostname : void 0, rpOrigins = typeof window !== "undefined" ? [window.location.origin] : void 0, signal: signal2 } = {} }, overrides) {
      if (!rpId) {
        return {
          data: null,
   ...
   */
  _register(_0, _1) {
    return __async(this, arguments, function* ({ friendlyName, webauthn: { rpId = typeof window !== "undefined" ? window.location.hostname : void 0, rpOrigins = typeof window !== "undefined" ? [window.location.origin] : void 0, signal: signal2 } = {} }, overrides) {
      if (!rpId) {
        return {
          data: null,
...
var fetchWithAuth = (supabaseKey, getAccessToken, customFetch) => {
  ...
  return (input2, init) => __async(null, null, function* () {
    ...
    return fetch$1(input2, _objectSpread23(_objectSpread23({}, init), {}, { headers }));
  });
};
...
// libs/core/src/lib/services/supabase-client.service.ts
var SUPABASE_CONFIG = new InjectionToken("SUPABASE_CONFIG");
var SUPABASE_CLIENT = new InjectionToken("SUPABASE_CLIENT");
var lockQueue = /* @__PURE__ */ new Map();
function withInAppLock(name, _acquireTimeout, fn) {
  return __async(this, null, function* () {
...
// libs/core/src/lib/services/membership.service.ts
var MembershipService = class _MembershipService {
  ...
  load(userId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("user_condominios").select("user_id, condominio_id, role, status, metadata, condominio:condominios(id, nome, subdomain, plan, subscription_status)").eq("user_id", userId).eq("status", "active");
      if (error) {
      ...
      const memberships = (data ?? []).map((row) => ({
        ...
        unidade: row.metadata?.unidade ?? void 0
      }));
...
// libs/core/src/lib/services/tenant.service.ts
var TENANT_STORAGE_KEY = "cm_current_tenant";
...
// libs/core/src/lib/services/context.service.ts
var CONTEXT_KEY = "cm_context";
var PLATFORM_LABELS = {
  morador: "Morador",
  porteiro: "Porteiro",
  admin: "Admin",
...
var APP_DEFAULT_ROUTE = {
  morador: "/home",
  porteiro: "/home",
  admin: "/dashboard",
  "master-admin": "/dashboard"
};
var APP_EXTERNAL_ROUTE = {
  morador: "/morador",
  porteiro: "/porteiro",
  admin: "/admin",
  "master-admin": "/master-admin"
};
var PLATFORM_ALLOWED_ROLES = {
...
var ContextService = class _ContextService {
  constructor() {
    this.state = inject(AuthState);
    this.tenants = inject(TenantService);
  }
  setContext(ctx) {
  ...
  isContextValid(ctx) {
    if (ctx.platform === "master-admin")
      return this.state.isMasterAdmin();
    if (this.state.isMasterAdmin() && ctx.condominioId)
      return true;
    if (!ctx.condominioId)
      return false;
    const allowed = PLATFORM_ALLOWED_ROLES[ctx.platform];
    return this.state.activeMemberships().some((m) => m.tenant.id === ctx.condominioId && allowed.includes(m.role) && m.ativo);
  }
  applyContext(ctx) {
    if (ctx.platform === "master-admin" || !ctx.condominioId)
      return;
  ...
  getAvailablePlatforms() {
    ...
    if (this.state.isMasterAdmin())
      platforms.add("master-admin");
  ...
  getTenantsForPlatform(platform) {
    if (platform === "master-admin")
      return [];
    const allowed = PLATFORM_ALLOWED_ROLES[platform];
    const seen = /* @__PURE__ */ new Set();
    return this.state.activeMemberships().filter((m) => allowed.includes(m.role) && !seen.has(m.tenant.id) && !!seen.add(m.tenant.id)).map((m) => m.tenant);
  }
  needsContextSelection() {
    const ctx = this.getContext();
    if (ctx && this.isContextValid(ctx))
      return false;
  ...
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ContextService, factory: _ContextService.\u0275fac, providedIn: "root" });
  }
};
(() => {
...
// libs/core/src/lib/auth/session.service.ts
var AUTH_APP_KEY = new InjectionToken("AUTH_APP_KEY");
var APP_DEFAULT_ROUTE2 = {
  morador: "/home",
  porteiro: "/home",
  admin: "/dashboard",
  "master-admin": "/dashboard"
};
var APP_EXTERNAL_ROUTE2 = {
  morador: "/morador",
  porteiro: "/porteiro",
  admin: "/admin",
  "master-admin": "/master-admin"
...
var SessionService = class _SessionService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.state = inject(AuthState);
    this.memberships = inject(MembershipService);
    this.tenants = inject(TenantService);
    this.ctxSvc = inject(ContextService);
    this.router = inject(Router);
    this.appKey = inject(AUTH_APP_KEY);
    this.authSub = null;
  ...
  initialize() {
    return __async(this, null, function* () {
      ...
      try {
        ...
        yield this.syncSession(session, { redirect: true, force: true });
      } catch (error) {
        this.clearState();
        this.state.error.set(error instanceof Error ? error.message : "N\xC3\xA3o foi poss\xC3\xADvel restaurar a sess\xC3\xA3o.");
      } finally {
  ...
  syncSession(_0) {
    return __async(this, arguments, function* (session, options = { redirect: false }) {
      const token = session?.access_token ?? null;
      if (!options.force && token && token === this.lastSessionToken) {
        if (options.redirect) {
          yield this.redirectForResolvedRole();
        }
        return;
      }
      if (!session?.user) {
      ...
      const profile = yield this.loadProfile(session.user);
      this.state.profile.set(profile);
      yield this.memberships.load(session.user.id);
  ...
  resolveTargetApp() {
    ...
    const currentAppMembership = memberships.find((membership) => this.roleIsAllowedInApp(membership.role, this.appKey));
    ...
    for (const role of ["ADMIN", "SINDICO", "CONSELHO", "PORTEIRO", "MORADOR"]) {
  ...
  redirectForResolvedRole() {
    return __async(this, null, function* () {
      ...
      if (!profile.isMasterAdmin && !memberships.length) {
        this.state.error.set("Seu usu\xC3\xA1rio n\xC3\xA3o possui condom\xC3\xADnio vinculado.");
      ...
      if (this.ctxSvc.needsContextSelection()) {
        ...
        if (currentPath !== "/select-context") {
          yield this.router.navigate(["/select-context"]);
        }
        return;
      }
  ...
  navigateToApp(targetApp) {
    if (targetApp === this.appKey) {
      const defaultRoute = APP_DEFAULT_ROUTE2[targetApp];
      const currentPath = window.location.pathname || "/";
      const routerPath = (this.router.url || currentPath).split("?")[0];
      const isAuthEntry = currentPath === "/" || currentPath === "/login" || currentPath === "/select-context" || currentPath === "/tenant-select" || routerPath === "/" || routerPath === "/login" || routerPath === "/select-context" || routerPath === "/tenant-select";
      if (isAuthEntry) {
    ...
    const crossIsAuthEntry = crossPath === "/" || crossPath === "/login" || crossPath === "/select-context" || crossPath === "/tenant-select" || crossRouter === "/" || crossRouter === "/login" || crossRouter === "/select-context" || crossRouter === "/tenant-select";
    if (crossIsAuthEntry) {
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED" || event === "USER_UPDATED") {
          yield this.syncSession(session, { redirect: true });
        }
      } catch (error) {
        this.clearState();
        this.state.error.set(error instanceof Error ? error.message : "N\xC3\xA3o foi poss\xC3\xADvel atualizar a sess\xC3\xA3o.");
      }
    });
  }
  loadProfile(user) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("users").select("id,email,nome,is_master_admin").eq("id", user.id).maybeSingle();
      if (error) {
      ...
      return {
        ...
        name: data?.nome ?? user.user_metadata?.["name"] ?? user.email ?? "Usu\xC3\xA1rio",
        isMasterAdmin: data?.is_master_admin ?? false
      };
    });
  }
  restoreOrSelectTenant() {
  ...
  pickDefaultMembership(memberships) {
    ...
    const priority = ["ADMIN", "SINDICO", "CONSELHO", "PORTEIRO", "MORADOR"];
  ...
  resolveAppUrl(targetApp) {
    const { protocol, hostname, port } = window.location;
    const targetPort = APP_DEV_PORT2[targetApp];
    if (hostname === "localhost" || hostname === "127.0.0.1") {
      if (targetPort && port !== targetPort) {
        return `${protocol}//${hostname}:${targetPort}`;
      }
    ...
    return APP_EXTERNAL_ROUTE2[targetApp];
  }
  static {
...
// libs/core/src/lib/auth/auth.service.ts
var AuthService = class _AuthService {
  ...
  init() {
    return __async(this, null, function* () {
      yield this.sessions.initialize();
    });
  }
  loginWithGoogle() {
    return __async(this, null, function* () {
      ...
      this.state.error.set(null);
      try {
        const { error } = yield this.supabase.auth.signInWithOAuth({
          provider: "google",
          options: {
        ...
        this.state.error.set(error instanceof Error ? error.message : "N\xC3\xA3o foi poss\xC3\xADvel iniciar o login com Google.");
  ...
  login(email, password) {
    return __async(this, null, function* () {
      ...
      this.state.error.set(null);
      try {
        const { data, error } = yield this.supabase.auth.signInWithPassword({ email, password });
        if (error) {
          throw error;
        }
        yield this.sessions.syncSession(data.session, { redirect: true, force: true });
      } catch (error) {
        this.state.error.set(error instanceof Error ? error.message : "N\xC3\xA3o foi poss\xC3\xADvel fazer login.");
      } finally {
        this.state.isLoading.set(false);
      }
    });
  ...
  getSession() {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.auth.getSession();
      if (error) {
        throw error;
      }
      return data.session;
...
// libs/core/src/lib/services/delivery.service.ts
var DELIVERY_COLUMNS = "id,condominio_id,morador_id,porteiro_id,recebido_por,unidade,transportadora,codigo_rastreamento,tipo,descricao,foto_url,status,data_retirada,quem_retirou,created_at,updated_at";
function mapRow(row) {
  return {
    id: row.id,
...
var DeliveryService = class _DeliveryService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.deliveries = signal([], ...ngDevMode ? [{ debugName: "deliveries" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
    ...
    this.channel = null;
    this.realtimeEvent = signal(null, ...ngDevMode ? [{ debugName: "realtimeEvent" }] : (
      /* istanbul ignore next */
      []
    ));
  }
  subscribeToTenant(condominioId) {
    this.stopRealtime();
    this.channel = this.supabase.channel(`entregas:tenant:${condominioId}`).on("postgres_changes", { event: "*", schema: "public", table: "entregas", filter: `condominio_id=eq.${condominioId}` }, (payload) => this.handleRealtimeEvent(payload)).subscribe();
  }
  subscribeForUser(userId) {
    this.stopRealtime();
    this.channel = this.supabase.channel(`entregas:user:${userId}`).on("postgres_changes", { event: "*", schema: "public", table: "entregas", filter: `morador_id=eq.${userId}` }, (payload) => this.handleRealtimeEvent(payload)).subscribe();
  }
  stopRealtime() {
    if (this.channel) {
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("entregas").select(DELIVERY_COLUMNS).eq("condominio_id", condominioId).is("deleted_at", null).order("created_at", { ascending: false }).limit(100);
  ...
  loadForUnit(condominioId, unidade) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("entregas").select(DELIVERY_COLUMNS).eq("condominio_id", condominioId).eq("unidade", unidade).is("deleted_at", null).order("created_at", { ascending: false }).limit(100);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadById(id) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("entregas").select(DELIVERY_COLUMNS).eq("id", id).is("deleted_at", null).single();
      if (error || !data)
        return null;
      return mapRow(data);
    });
  }
  marcarRetirada(id, quemRetirou) {
    return __async(this, null, function* () {
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const { error } = yield this.supabase.from("entregas").update({ status: "retirada", data_retirada: now, quem_retirou: quemRetirou }).eq("id", id);
      if (error) {
        this.error.set(error.message);
        return false;
      }
      this.deliveries.update((list) => list.map((d) => d.id === id ? __spreadProps(__spreadValues({}, d), { status: "retirada", dataRetirada: now, quemRetirou }) : d));
      return true;
    });
  }
  loadForUser(userId, condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      let query = this.supabase.from("entregas").select(DELIVERY_COLUMNS).eq("morador_id", userId).is("deleted_at", null).order("created_at", { ascending: false }).limit(100);
      if (condominioId)
  ...
  create(input2) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("entregas").insert({
        condominio_id: input2.condominioId,
        unidade: input2.unidade,
        morador_id: input2.moradorId ?? null,
        tipo: input2.tipo,
        transportadora: input2.transportadora ?? null,
        descricao: input2.descricao ?? null,
        status: "pendente"
      }).select(DELIVERY_COLUMNS).single();
      if (error || !data) {
        this.error.set(error?.message ?? "Erro ao criar entrega");
        return null;
      }
  ...
  markAsReceived(id, userId) {
    return __async(this, null, function* () {
      return this.marcarRetirada(id, userId);
    });
  }
  loadResidents(condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("user_condominios").select("metadata, user:users(id, nome)").eq("condominio_id", condominioId).eq("role", "morador").eq("status", "active");
      if (error || !data)
        return [];
  ...
  static {
    this.\u0275fac = function DeliveryService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _DeliveryService)();
    };
  }
  static {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DeliveryService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// libs/core/src/lib/services/occurrence.service.ts
var OCCURRENCE_COLUMNS = "id,condominio_id,user_id,porteiro_id,titulo,tipo,descricao,local,data_ocorrido,status,resolvida_por,resolucao,created_at,updated_at";
var OCCURRENCE_IMAGE_COLUMNS = "id,ocorrencia_id,image_url,ordem,created_at";
...
var OccurrenceService = class _OccurrenceService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.occurrences = signal([], ...ngDevMode ? [{ debugName: "occurrences" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
  ...
  subscribeToTenant(condominioId) {
    this.stopRealtime();
    this.channel = this.supabase.channel(`ocorrencias:tenant:${condominioId}`).on("postgres_changes", { event: "*", schema: "public", table: "ocorrencias", filter: `condominio_id=eq.${condominioId}` }, (payload) => this.handleRealtimeEvent(payload)).subscribe();
  }
  subscribeForUser(userId) {
    this.stopRealtime();
    this.channel = this.supabase.channel(`ocorrencias:user:${userId}`).on("postgres_changes", { event: "*", schema: "public", table: "ocorrencias", filter: `user_id=eq.${userId}` }, (payload) => this.handleRealtimeEvent(payload)).subscribe();
  }
  stopRealtime() {
    if (this.channel) {
      this.supabase.removeChannel(this.channel);
  ...
  handleRealtimeEvent(payload) {
    const { eventType, new: newRow, old: oldRow } = payload;
    if (eventType === "INSERT") {
      const occurrence = mapRow2(newRow);
  ...
  loadMyOccurrences(condominioId, userId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("ocorrencias").select(OCCURRENCE_COLUMNS).eq("condominio_id", condominioId).eq("user_id", userId).order("created_at", { ascending: false }).limit(100);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
      }
      this.occurrences.set((data ?? []).map(mapRow2));
    });
  }
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("ocorrencias").select(OCCURRENCE_COLUMNS).eq("condominio_id", condominioId).order("created_at", { ascending: false }).limit(100);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadById(id) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("ocorrencias").select(OCCURRENCE_COLUMNS).eq("id", id).single();
      if (error || !data)
        return null;
      return mapRow2(data);
    });
  }
  loadImages(ocorrenciaId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("ocorrencia_imagens").select(OCCURRENCE_IMAGE_COLUMNS).eq("ocorrencia_id", ocorrenciaId).order("ordem").limit(20);
      if (error || !data)
        return [];
  ...
  create(input2) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("ocorrencias").insert({
        condominio_id: input2.condominioId,
        user_id: input2.userId,
        titulo: input2.titulo,
        tipo: input2.tipo,
        descricao: input2.descricao,
        local: input2.local ?? null,
        status: "aberta"
      }).select(OCCURRENCE_COLUMNS).single();
      if (error || !data) {
        this.error.set(error?.message ?? "Erro ao criar ocorr\xC3\xAAncia");
        return null;
      }
      const occurrence = mapRow2(data);
      this.occurrences.update((list) => [occurrence, ...list]);
      return occurrence;
    });
  }
  addImage(ocorrenciaId, imageUrl, ordem = 0) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("ocorrencia_imagens").insert({ ocorrencia_id: ocorrenciaId, image_url: imageUrl, ordem });
      return !error;
    });
  }
  updateStatus(id, status, resolucao) {
    return __async(this, null, function* () {
      const patch = { status };
      if (resolucao !== void 0)
        patch.resolucao = resolucao;
      const { error } = yield this.supabase.from("ocorrencias").update(patch).eq("id", id);
      if (error) {
        this.error.set(error.message);
        return false;
      }
      this.occurrences.update((list) => list.map((o) => o.id === id ? __spreadProps(__spreadValues({}, o), { status, resolucao: resolucao ?? o.resolucao }) : o));
      return true;
    });
  }
  static {
    this.\u0275fac = function OccurrenceService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _OccurrenceService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _OccurrenceService, factory: _OccurrenceService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(OccurrenceService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// libs/core/src/lib/services/notification.service.ts
...
// libs/core/src/lib/services/upload.service.ts
var UploadService = class _UploadService {
  ...
  upload(bucket, path, file) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.storage.from(bucket).upload(path, file, { upsert: false, contentType: file.type });
      if (error)
        throw new Error(error.message);
  ...
  buildPath(condominioId, ocorrenciaId, file) {
    const uuid = crypto.randomUUID();
    const ext = file.name.split(".").pop() ?? "jpg";
    return `${condominioId}/${ocorrenciaId}/${uuid}.${ext}`;
  }
  compress(file, maxWidth = 1920, quality = 0.82) {
    return new Promise((resolve, reject) => {
      const img = new Image();
...
// libs/core/src/lib/services/reservation.service.ts
var EQUIPAMENTO_COLUMNS = "id,condominio_id,nome,descricao,capacidade,regras,horario_inicio,horario_fim,ativo,image_url,created_at,updated_at";
var RESERVATION_COLUMNS = "id,condominio_id,equipamento_id,morador_id,data,hora_inicio,hora_fim,motivo,num_convidados,status,google_event_id,google_calendar_id,created_at,updated_at";
...
function mapRow3(row) {
  return {
    ...
    data: row.data,
    horaInicio: row.hora_inicio?.slice(0, 5) ?? row.hora_inicio,
    horaFim: row.hora_fim?.slice(0, 5) ?? row.hora_fim,
    ...
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    equipamento: row.equipamento ? { id: row.equipamento.id, nome: row.equipamento.nome, imageUrl: row.equipamento.image_url ?? null } : void 0,
    morador: row.morador ? { id: row.morador.id, nome: row.morador.nome, email: row.morador.email } : void 0
  };
}
var ReservationService = class _ReservationService {
  constructor() {
    ...
    this.equipamentos = signal([], ...ngDevMode ? [{ debugName: "equipamentos" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
  ...
  subscribeToTenant(condominioId) {
    this.stopRealtime();
    this.channel = this.supabase.channel(`reservas:tenant:${condominioId}`).on("postgres_changes", { event: "*", schema: "public", table: "reservas", filter: `condominio_id=eq.${condominioId}` }, (payload) => this.handleRealtimeEvent(payload)).subscribe();
  }
  subscribeForUser(userId) {
    this.stopRealtime();
    this.channel = this.supabase.channel(`reservas:user:${userId}`).on("postgres_changes", { event: "*", schema: "public", table: "reservas", filter: `morador_id=eq.${userId}` }, (payload) => this.handleRealtimeEvent(payload)).subscribe();
  }
  stopRealtime() {
    if (this.channel) {
      this.supabase.removeChannel(this.channel);
  ...
  loadEquipamentos(condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("equipamentos").select(EQUIPAMENTO_COLUMNS).eq("condominio_id", condominioId).eq("ativo", true).order("nome").limit(100);
      if (error) {
        this.error.set(error.message);
        return;
      }
  ...
  loadAllEquipamentos(condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("equipamentos").select(EQUIPAMENTO_COLUMNS).eq("condominio_id", condominioId).order("nome").limit(100);
      if (error) {
        this.error.set(error.message);
        return;
      }
  ...
  createEquipamento(input2) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("equipamentos").insert({
        condominio_id: input2.condominioId,
        nome: input2.nome,
        descricao: input2.descricao ?? null,
        capacidade: input2.capacidade ?? null,
        horario_inicio: input2.horarioInicio ?? "08:00",
        horario_fim: input2.horarioFim ?? "22:00"
      }).select(EQUIPAMENTO_COLUMNS).single();
      if (error || !data) {
        this.error.set(error?.message ?? "Erro ao criar equipamento");
        return null;
      }
  ...
  toggleEquipamento(id, ativo) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("equipamentos").update({ ativo }).eq("id", id);
      if (error) {
        this.error.set(error.message);
        return false;
      }
      this.equipamentos.update((list) => list.map((e) => e.id === id ? __spreadProps(__spreadValues({}, e), { ativo }) : e));
      return true;
    });
  }
  // â”€â”€ Reservas â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  loadMyReservations(condominioId, userId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("reservas").select(`${RESERVATION_COLUMNS}, equipamento:equipamentos(id,nome,image_url)`).eq("condominio_id", condominioId).eq("morador_id", userId).is("deleted_at", null).order("data", { ascending: false }).order("hora_inicio", { ascending: false }).limit(100);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
      }
      this.reservations.set((data ?? []).map(mapRow3));
      this.subscribeForUser(userId);
    });
  }
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("reservas").select(`${RESERVATION_COLUMNS}, equipamento:equipamentos(id,nome,image_url), morador:users(id,nome,email)`).eq("condominio_id", condominioId).is("deleted_at", null).order("data", { ascending: false }).order("hora_inicio", { ascending: false }).limit(100);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadForDate(equipamentoId, data) {
    return __async(this, null, function* () {
      const { data: rows, error } = yield this.supabase.from("reservas").select(RESERVATION_COLUMNS).eq("equipamento_id", equipamentoId).eq("data", data).eq("status", "confirmada").is("deleted_at", null).limit(100);
      if (error || !rows)
        return [];
  ...
  create(input2) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("reservas").insert({
        condominio_id: input2.condominioId,
        equipamento_id: input2.equipamentoId,
        morador_id: input2.moradorId,
        data: input2.data,
        hora_inicio: input2.horaInicio,
        hora_fim: input2.horaFim,
        motivo: input2.motivo ?? null,
        num_convidados: input2.numConvidados ?? 0,
        status: "confirmada"
      }).select(`${RESERVATION_COLUMNS}, equipamento:equipamentos(id,nome,image_url)`).single();
      if (error || !data) {
        this.error.set(error?.message ?? "Erro ao criar reserva");
        return null;
      }
      const reservation = mapRow3(data);
      this.reservations.update((list) => [reservation, ...list]);
      return reservation;
    });
  }
  cancel(id) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("reservas").update({ status: "cancelada" }).eq("id", id);
      if (error) {
        this.error.set(error.message);
        return false;
      }
      this.reservations.update((list) => list.map((r) => r.id === id ? __spreadProps(__spreadValues({}, r), { status: "cancelada" }) : r));
      return true;
    });
  }
  updateGoogleEventId(id, googleEventId, googleCalendarId) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("reservas").update({ google_event_id: googleEventId, google_calendar_id: googleCalendarId }).eq("id", id);
      if (error)
        return false;
      this.reservations.update((list) => list.map((r) => r.id === id ? __spreadProps(__spreadValues({}, r), { googleEventId, googleCalendarId }) : r));
      return true;
    });
  }
  static {
    this.\u0275fac = function ReservationService_Factory(__ngFactoryType__) {
...
// libs/core/src/lib/services/google-calendar.service.ts
var STORAGE_KEY2 = "cm_google_tokens";
...
var GoogleCalendarService = class _GoogleCalendarService {
  ...
  saveTokens(tokens) {
    localStorage.setItem(STORAGE_KEY2, JSON.stringify(tokens));
  ...
  storeTokensFromOAuth(accessToken, refreshToken, expiresIn = 3600) {
    this.saveTokens({
      accessToken,
      refreshToken,
  ...
  getValidAccessToken() {
    return __async(this, null, function* () {
      ...
      if (!tokens)
        return null;
      if (Date.now() < tokens.expiresAt - 6e4) {
      ...
      try {
        const { data, error } = yield this.supabase.functions.invoke("google-token-refresh", {
        ...
        this.saveTokens(__spreadProps(__spreadValues({}, tokens), {
  ...
  createEvent(reservation, equipamento) {
    return __async(this, null, function* () {
      const token = yield this.getValidAccessToken();
      if (!token)
        return null;
      const event = {
      ...
      try {
        const resp = yield fetch(`${CALENDAR_API}/calendars/primary/events`, {
          method: "POST",
          headers: {
  ...
  deleteEvent(eventId) {
    return __async(this, null, function* () {
      ...
      if (!token)
        return false;
      try {
        const resp = yield fetch(`${CALENDAR_API}/calendars/primary/events/${eventId}`, {
          method: "DELETE",
  ...
  updateEvent(eventId, reservation, equipamento) {
    return __async(this, null, function* () {
      const token = yield this.getValidAccessToken();
      if (!token)
        return false;
  ...
  static {
    this.\u0275fac = function GoogleCalendarService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _GoogleCalendarService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _GoogleCalendarService, factory: _GoogleCalendarService.\u0275fac, providedIn: "root" });
  }
};
(() => {
...
// libs/core/src/lib/services/feature.service.ts
var FALLBACK = {
  free: ["entregas"],
...
var FeatureService = class _FeatureService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.authState = inject(AuthState);
    this._enabled = signal(new Set(FALLBACK["free"]), ...ngDevMode ? [{ debugName: "_enabled" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
    ...
    effect(() => {
      ...
      if (plano)
        void this.loadForPlan(plano);
    });
  }
  hasFeature(code) {
    return this._enabled().has(code);
  }
  loadForPlan(planNome) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      try {
        const { data, error } = yield this.supabase.from("plano_features").select("feature_code").eq("plano_nome", planNome).eq("enabled", true);
        if (error || !data?.length) {
          this._applyFallback(planNome);
          return;
        }
        this._enabled.set(new Set(data.map((r) => r.feature_code)));
      } catch (e) {
        this._applyFallback(planNome);
      } finally {
        this.isLoading.set(false);
  ...
  _applyFallback(planNome) {
    const codes = FALLBACK[planNome] ?? FALLBACK["free"];
    this._enabled.set(new Set(codes));
  }
  static {
    this.\u0275fac = function FeatureService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _FeatureService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FeatureService, factory: _FeatureService.\u0275fac, providedIn: "root" });
  }
};
(() => {
...
// libs/core/src/lib/services/billing.service.ts
var DEFAULT_PLANOS = [
  {
    nome: "free",
    label: "Gratis",
...
var BillingService = class _BillingService {
  constructor() {
    ...
    this.subscription = signal(null, ...ngDevMode ? [{ debugName: "subscription" }] : (
      /* istanbul ignore next */
      []
    ));
    this.planos = signal(DEFAULT_PLANOS, ...ngDevMode ? [{ debugName: "planos" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
    ...
    this.planNome = computed(() => this.subscription()?.plano_nome ?? "free", ...ngDevMode ? [{ debugName: "planNome" }] : (
      /* istanbul ignore next */
      []
    ));
    ...
    this.isPastDue = computed(() => this.subscription()?.status === "PAST_DUE", ...ngDevMode ? [{ debugName: "isPastDue" }] : (
      /* istanbul ignore next */
      []
    ));
    this.canAccessOccurrences = computed(() => this.isActive() && this.featureSvc.hasFeature("ocorrencias"), ...ngDevMode ? [{ debugName: "canAccessOccurrences" }] : (
      /* istanbul ignore next */
      []
    ));
    this.canAccessReservations = computed(() => this.isActive() && this.featureSvc.hasFeature("reservas"), ...ngDevMode ? [{ debugName: "canAccessReservations" }] : (
      /* istanbul ignore next */
      []
    ));
    this.canAccessMarketplace = computed(() => this.isActive() && this.featureSvc.hasFeature("marketplace"), ...ngDevMode ? [{ debugName: "canAccessMarketplace" }] : (
      /* istanbul ignore next */
      []
    ));
  }
  ...
  subscribeToPlan(condominioId, planName) {
    return __async(this, null, function* () {
      const origin = window.location.origin;
      const successUrl = `${origin}/billing?success=1`;
      const cancelUrl = `${origin}/billing?canceled=1`;
      const { data, error } = yield this.supabase.functions.invoke("create-checkout-session", {
  ...
  getCurrentSubscription(condominioId) {
    return __async(this, null, function* () {
  ...
  subscriptionFromCurrentTenant(condominioId) {
    const tenant = this.authState.currentTenant();
    if (!tenant || tenant.id !== condominioId)
      return null;
    return {
...
// libs/core/src/lib/services/announcement.service.ts
var AnnouncementService = class _AnnouncementService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.announcements = signal([], ...ngDevMode ? [{ debugName: "announcements" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("avisos").select("id,condominio_id,titulo,mensagem,prioridade,publicado_em,expira_em,image_url").eq("condominio_id", condominioId).eq("status", "publicado").order("publicado_em", { ascending: false }).limit(50);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
      ...
      this.announcements.set((data ?? []).map(mapAnnouncement));
    });
  }
  loadById(id) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("avisos").select("id,condominio_id,titulo,mensagem,prioridade,publicado_em,expira_em,image_url").eq("id", id).eq("status", "publicado").maybeSingle();
      if (error || !data) {
        return null;
      }
  ...
  static {
    this.\u0275fac = function AnnouncementService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _AnnouncementService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _AnnouncementService, factory: _AnnouncementService.\u0275fac, providedIn: "root" });
  }
};
(() => {
...
// libs/core/src/lib/services/marketplace.service.ts
var MarketplaceService = class _MarketplaceService {
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("classificados").select("id,condominio_id,morador_id,titulo,descricao,preco,categoria,imagens,status,created_at,vendedor:users(nome,telefone)").eq("condominio_id", condominioId).eq("status", "ativo").order("created_at", { ascending: false }).limit(50);
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        this.items.set([]);
      ...
      this.items.set((data ?? []).map(mapMarketplaceItem));
    });
  }
  loadById(id) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("classificados").select("id,condominio_id,morador_id,titulo,descricao,preco,categoria,imagens,status,created_at,vendedor:users(nome,telefone)").eq("id", id).eq("status", "ativo").maybeSingle();
      if (error || !data) {
        return null;
      }
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(MarketplaceService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();
function mapMarketplaceItem(row) {
  ...
  return {
    ...
    whatsapp: vendedor?.telefone ?? void 0,
    createdAt: row.created_at
  };
}
...
// libs/core/src/lib/auth/global-login.component.ts
function GlobalLoginComponent_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
...
var GlobalLoginComponent = class _GlobalLoginComponent {
  constructor() {
  ...
  static {
    this.\u0275fac = function GlobalLoginComponent_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _GlobalLoginComponent)();
    };
  }
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _GlobalLoginComponent, selectors: [["cm-global-login"]], decls: 29, vars: 8, consts: [[1, "login"], [1, "login__container"], [1, "login__hero"], [1, "login__eyebrow"], [1, "login__title"], [1, "login__subtitle"], [1, "login__card"], [1, "login__form"], [1, "login__error"], ["type", "button", 1, "login__btn", "login__btn--google", 3, "click", "disabled"], [1, "login__divider"], [1, "login__field"], [1, "login__label"], ["type", "email", "placeholder", "voce@condominio.com", 1, "login__input", 3, "ngModelChange", "ngModel", "disabled"], ["type", "password", "placeholder", "Digite sua senha", 1, "login__input", 3, "ngModelChange", "keyup.enter", "ngModel", "disabled"], ["type", "button", 1, "login__btn", "login__btn--accent", 3, "click", "disabled"]], template: function GlobalLoginComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275elementStart(0, "div", 0)(1, "div", 1)(2, "div", 2)(3, "p", 3);
        \u0275\u0275text(4, "Autentica\xE7\xE3o Global");
        \u0275\u0275elementEnd();
        ...
        \u0275\u0275text(10, "Uma \xFAnica sess\xE3o para morador, portaria, administra\xE7\xE3o e master-admin.");
        \u0275\u0275elementEnd()();
        \u0275\u0275elementStart(11, "div", 6)(12, "div", 7);
        \u0275\u0275conditionalCreate(13, GlobalLoginComponent_Conditional_13_Template, 2, 1, "p", 8);
        \u0275\u0275elementStart(14, "button", 9);
        \u0275\u0275listener("click", function GlobalLoginComponent_Template_button_click_14_listener() {
          return ctx.loginWithGoogle();
        });
        \u0275\u0275text(15, " Continuar com Google ");
        \u0275\u0275elementEnd();
        \u0275\u0275elementStart(16, "div", 10)(17, "span");
        \u0275\u0275text(18, "OU");
        \u0275\u0275elementEnd()();
        \u0275\u0275elementStart(19, "label", 11)(20, "span", 12);
        \u0275\u0275text(21, "E-mail");
        \u0275\u0275elementEnd();
        \u0275\u0275elementStart(22, "input", 13);
        \u0275\u0275twoWayListener("ngModelChange", function GlobalLoginComponent_Template_input_ngModelChange_22_listener($event) {
        ...
        \u0275\u0275twoWayListener("ngModelChange", function GlobalLoginComponent_Template_input_ngModelChange_26_listener($event) {
          \u0275\u0275twoWayBindingSet(ctx.password, $event) || (ctx.password = $event);
          return $event;
        });
        \u0275\u0275listener("keyup.enter", function GlobalLoginComponent_Template_input_keyup_enter_26_listener() {
          return ctx.loginWithEmail();
        });
        \u0275\u0275elementEnd()();
        \u0275\u0275elementStart(27, "button", 15);
        \u0275\u0275listener("click", function GlobalLoginComponent_Template_button_click_27_listener() {
          return ctx.loginWithEmail();
        });
        \u0275\u0275text(28);
        \u0275\u0275elementEnd()()()()();
      ...
      if (rf & 2) {
        \u0275\u0275advance(13);
        \u0275\u0275conditional(ctx.state.error() ? 13 : -1);
        \u0275\u0275advance();
        ...
        \u0275\u0275property("disabled", ctx.state.isLoading());
        \u0275\u0275advance();
        \u0275\u0275property("disabled", ctx.state.isLoading() || !ctx.email.trim() || !ctx.password.trim());
        \u0275\u0275advance();
        \u0275\u0275textInterpolate1(" ", ctx.state.isLoading() ? "Aguarde..." : "Entrar com e-mail", " ");
      }
    }, dependencies: [FormsModule, DefaultValueAccessor, NgControlStatus, NgModel], styles: ['\n.login[_ngcontent-%COMP%] {\n  min-height: 100dvh;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 100%;\n  overflow-x: hidden;\n  padding: var(--s-9, 3rem) var(--s-6, 1.5rem);\n  box-sizing: border-box;\n  background:\n    radial-gradient(\n      circle at top left,\n      rgba(45, 106, 79, 0.12),\n      transparent 28rem),\n    linear-gradient(\n      180deg,\n      #f7f4ee 0%,\n      #f3efe7 100%);\n}\n.login__container[_ngcontent-%COMP%] {\n  width: min(100%, 30rem);\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  gap: var(--s-8, 2rem);\n}\n.login__hero[_ngcontent-%COMP%] {\n  display: flex;\n  flex-direction: column;\n  gap: var(--s-4, 1rem);\n  width: 100%;\n  text-align: center;\n  align-items: center;\n}\n.login__eyebrow[_ngcontent-%COMP%] {\n  margin: 0;\n  font-size: 11px;\n  font-weight: 700;\n  text-transform: uppercase;\n  letter-spacing: 0.14em;\n  color: #6b7280;\n}\n.login__title[_ngcontent-%COMP%] {\n  margin: 0;\n  color: #111827;\n  font-family: var(--font-serif, Georgia, serif);\n  font-size: clamp(2.4rem, 7vw, 4.4rem);\n  font-weight: 400;\n  line-height: 0.98;\n}\n.login__title[_ngcontent-%COMP%]   em[_ngcontent-%COMP%] {\n  color: #2d6a4f;\n  font-style: italic;\n}\n.login__subtitle[_ngcontent-%COMP%] {\n  margin: 0;\n  max-width: 30rem;\n  color: #4b5563;\n  font-size: 1rem;\n  line-height: 1.6;\n}\n.login__card[_ngcontent-%COMP%] {\n  width: 100%;\n  max-width: 100%;\n  background: rgba(255, 255, 255, 0.88);\n  border: 1px solid rgba(255, 255, 255, 0.75);\n  border-radius: 1.5rem;\n  box-shadow: 0 24px 60px rgba(15, 23, 42, 0.12);\n  -webkit-backdrop-filter: blur(14px);\n  backdrop-filter: blur(14px);\n  padding: 1.5rem;\n  box-sizing: border-box;\n}\n.login__form[_ngcontent-%COMP%] {\n  display: flex;\n  flex-direction: column;\n  gap: var(--s-4, 1rem);\n}\n.login__field[_ngcontent-%COMP%] {\n  display: flex;\n  flex-direction: column;\n  gap: var(--s-2, 0.5rem);\n}\n.login__label[_ngcontent-%COMP%] {\n  color: #6b7280;\n  font-size: 12px;\n  font-weight: 700;\n  letter-spacing: 0.06em;\n  text-transform: uppercase;\n}\n.login__input[_ngcontent-%COMP%] {\n  width: 100%;\n  box-sizing: border-box;\n  border: 1px solid #d1d5db;\n  border-radius: 0.9rem;\n  padding: 0.95rem 1rem;\n  background: #ffffff;\n  color: #111827;\n  font-size: 0.98rem;\n  outline: none;\n  transition: border-color 120ms ease, box-shadow 120ms ease;\n}\n.login__input[_ngcontent-%COMP%]:focus {\n  border-color: #2d6a4f;\n  box-shadow: 0 0 0 4px rgba(45, 106, 79, 0.12);\n}\n.login__btn[_ngcontent-%COMP%] {\n  width: 100%;\n  box-sizing: border-box;\n  border: none;\n  border-radius: 999px;\n  padding: 0.95rem 1.15rem;\n  font-size: 0.98rem;\n  font-weight: 700;\n  cursor: pointer;\n  transition:\n    transform 120ms ease,\n    box-shadow 120ms ease,\n    opacity 120ms ease;\n}\n.login__btn[_ngcontent-%COMP%]:disabled {\n  cursor: not-allowed;\n  opacity: 0.7;\n}\n.login__btn[_ngcontent-%COMP%]:not(:disabled):hover {\n  transform: translateY(-1px);\n}\n.login__btn--google[_ngcontent-%COMP%] {\n  background: #ffffff;\n  color: #111827;\n  border: 1px solid #d1d5db;\n  box-shadow: 0 10px 24px rgba(15, 23, 42, 0.08);\n}\n.login__btn--accent[_ngcontent-%COMP%] {\n  background:\n    linear-gradient(\n      135deg,\n      #2d6a4f,\n      #1f4d39);\n  color: #ffffff;\n  box-shadow: 0 16px 30px rgba(45, 106, 79, 0.24);\n}\n.login__divider[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 0.8rem;\n  color: #9ca3af;\n  font-size: 12px;\n  font-weight: 700;\n  letter-spacing: 0.1em;\n}\n.login__divider[_ngcontent-%COMP%]::before, \n.login__divider[_ngcontent-%COMP%]::after {\n  content: "";\n  flex: 1;\n  height: 1px;\n  background: #e5e7eb;\n}\n.login__error[_ngcontent-%COMP%] {\n  margin: 0;\n  padding: 0.85rem 1rem;\n  border-radius: 0.9rem;\n  background: #fef2f2;\n  color: #b91c1c;\n  font-size: 0.92rem;\n}\n/*# sourceMappingURL=global-login.component.css.map */'], changeDetection: 0 });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(GlobalLoginComponent, [{
    type: Component,
    args: [{ selector: "cm-global-login", standalone: true, imports: [FormsModule], changeDetection: ChangeDetectionStrategy.OnPush, template: `<div class="login">
  <div class="login__container">
    <div class="login__hero">
      <p class="login__eyebrow">Autentica\xE7\xE3o Global</p>
      <h1 class="login__title">Acesse o <em>CondoMais</em></h1>
      <p class="login__subtitle">Uma \xFAnica sess\xE3o para morador, portaria, administra\xE7\xE3o e master-admin.</p>
    </div>
    ...
    <div class="login__card">
      <div class="login__form">
        ...
        <label class="login__field">
          <span class="login__label">Senha</span>
          <input
            class="login__input"
            type="password"
            [(ngModel)]="password"
        ...
        <button
          class="login__btn login__btn--accent"
          type="button"
          (click)="loginWithEmail()"
          [disabled]="state.isLoading() || !email.trim() || !password.trim()"
        >
          {{ state.isLoading() ? 'Aguarde...' : 'Entrar com e-mail' }}
        </button>
      </div>
    </div>
  </div>
</div>
`, styles: ['/* libs/core/src/lib/auth/global-login.component.css */\n.login {\n  min-height: 100dvh;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  width: 100%;\n  overflow-x: hidden;\n  padding: var(--s-9, 3rem) var(--s-6, 1.5rem);\n  box-sizing: border-box;\n  background:\n    radial-gradient(\n      circle at top left,\n      rgba(45, 106, 79, 0.12),\n      transparent 28rem),\n    linear-gradient(\n      180deg,\n      #f7f4ee 0%,\n      #f3efe7 100%);\n}\n.login__container {\n  width: min(100%, 30rem);\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  gap: var(--s-8, 2rem);\n}\n.login__hero {\n  display: flex;\n  flex-direction: column;\n  gap: var(--s-4, 1rem);\n  width: 100%;\n  text-align: center;\n  align-items: center;\n}\n.login__eyebrow {\n  margin: 0;\n  font-size: 11px;\n  font-weight: 700;\n  text-transform: uppercase;\n  letter-spacing: 0.14em;\n  color: #6b7280;\n}\n.login__title {\n  margin: 0;\n  color: #111827;\n  font-family: var(--font-serif, Georgia, serif);\n  font-size: clamp(2.4rem, 7vw, 4.4rem);\n  font-weight: 400;\n  line-height: 0.98;\n}\n.login__title em {\n  color: #2d6a4f;\n  font-style: italic;\n}\n.login__subtitle {\n  margin: 0;\n  max-width: 30rem;\n  color: #4b5563;\n  font-size: 1rem;\n  line-height: 1.6;\n}\n.login__card {\n  width: 100%;\n  max-width: 100%;\n  background: rgba(255, 255, 255, 0.88);\n  border: 1px solid rgba(255, 255, 255, 0.75);\n  border-radius: 1.5rem;\n  box-shadow: 0 24px 60px rgba(15, 23, 42, 0.12);\n  -webkit-backdrop-filter: blur(14px);\n  backdrop-filter: blur(14px);\n  padding: 1.5rem;\n  box-sizing: border-box;\n}\n.login__form {\n  display: flex;\n  flex-direction: column;\n  gap: var(--s-4, 1rem);\n}\n.login__field {\n  display: flex;\n  flex-direction: column;\n  gap: var(--s-2, 0.5rem);\n}\n.login__label {\n  color: #6b7280;\n  font-size: 12px;\n  font-weight: 700;\n  letter-spacing: 0.06em;\n  text-transform: uppercase;\n}\n.login__input {\n  width: 100%;\n  box-sizing: border-box;\n  border: 1px solid #d1d5db;\n  border-radius: 0.9rem;\n  padding: 0.95rem 1rem;\n  background: #ffffff;\n  color: #111827;\n  font-size: 0.98rem;\n  outline: none;\n  transition: border-color 120ms ease, box-shadow 120ms ease;\n}\n.login__input:focus {\n  border-color: #2d6a4f;\n  box-shadow: 0 0 0 4px rgba(45, 106, 79, 0.12);\n}\n.login__btn {\n  width: 100%;\n  box-sizing: border-box;\n  border: none;\n  border-radius: 999px;\n  padding: 0.95rem 1.15rem;\n  font-size: 0.98rem;\n  font-weight: 700;\n  cursor: pointer;\n  transition:\n    transform 120ms ease,\n    box-shadow 120ms ease,\n    opacity 120ms ease;\n}\n.login__btn:disabled {\n  cursor: not-allowed;\n  opacity: 0.7;\n}\n.login__btn:not(:disabled):hover {\n  transform: translateY(-1px);\n}\n.login__btn--google {\n  background: #ffffff;\n  color: #111827;\n  border: 1px solid #d1d5db;\n  box-shadow: 0 10px 24px rgba(15, 23, 42, 0.08);\n}\n.login__btn--accent {\n  background:\n    linear-gradient(\n      135deg,\n      #2d6a4f,\n      #1f4d39);\n  color: #ffffff;\n  box-shadow: 0 16px 30px rgba(45, 106, 79, 0.24);\n}\n.login__divider {\n  display: flex;\n  align-items: center;\n  gap: 0.8rem;\n  color: #9ca3af;\n  font-size: 12px;\n  font-weight: 700;\n  letter-spacing: 0.1em;\n}\n.login__divider::before,\n.login__divider::after {\n  content: "";\n  flex: 1;\n  height: 1px;\n  background: #e5e7eb;\n}\n.login__error {\n  margin: 0;\n  padding: 0.85rem 1rem;\n  border-radius: 0.9rem;\n  background: #fef2f2;\n  color: #b91c1c;\n  font-size: 0.92rem;\n}\n/*# sourceMappingURL=global-login.component.css.map */\n'] }]
  }], null, null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(GlobalLoginComponent, { className: "GlobalLoginComponent", filePath: "libs/core/src/lib/auth/global-login.component.ts", lineNumber: 14 });
})();

// libs/core/src/lib/directives/has-feature.directive.ts
var HasFeatureDirective = class _HasFeatureDirective {
  constructor() {
    this.featureSvc = inject(FeatureService);
    this.vcr = inject(ViewContainerRef);
    this.tpl = inject(TemplateRef);
    this.hasView = false;
    this.appHasFeature = input.required(...ngDevMode ? [{ debugName: "appHasFeature" }] : (
      /* istanbul ignore next */
      []
    ));
    effect(() => {
  ...
  static {
    this.\u0275fac = function HasFeatureDirective_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _HasFeatureDirective)();
    };
  }
  static {
    this.\u0275dir = /* @__PURE__ */ \u0275\u0275defineDirective({ type: _HasFeatureDirective, selectors: [["", "appHasFeature", ""]], inputs: { appHasFeature: [1, "appHasFeature"] } });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(HasFeatureDirective, [{
    type: Directive,
    args: [{ selector: "[appHasFeature]", standalone: true }]
  }], () => [], { appHasFeature: [{ type: Input, args: [{ isSignal: true, alias: "appHasFeature", required: true }] }] });
})();

// libs/core/src/lib/services/user-management.service.ts
var UserManagementService = class _UserManagementService {
  ...
  getUsersPage(request) {
    return __async(this, null, function* () {
      ...
      let usersQuery = this.supabase.from("users").select("id,email,nome,is_master_admin,created_at", { count: "exact" });
      const filter = this.toSearchFilter(request.globalFilter);
      if (filter) {
        usersQuery = usersQuery.or(`nome.ilike.%${filter}%,email.ilike.%${filter}%`);
      }
      const usersRes = yield usersQuery.order(sortColumn, { ascending: request.sortOrder !== -1 }).range(first, last);
      if (usersRes.error) {
        throw new Error(usersRes.error.message);
      }
      const users = usersRes.data ?? [];
      const userIds = users.map((user) => user.id);
      const counts = yield this.getMembershipCounts(userIds);
      return {
  ...
  getUsers() {
    return __async(this, null, function* () {
      const page = yield this.getUsersPage({
        first: 0,
        rows: 100,
        sortField: "name",
        sortOrder: 1
  ...
  getMembershipCounts(userIds) {
    return __async(this, null, function* () {
      const counts = /* @__PURE__ */ new Map();
      if (!userIds.length) {
        return counts;
      }
      const membershipsRes = yield this.supabase.from("user_condominios").select("user_id,condominio_id,status").in("user_id", userIds).eq("status", "active");
      if (membershipsRes.error) {
        throw new Error(membershipsRes.error.message);
      }
      for (const membership of membershipsRes.data ?? []) {
        const userMemberships = counts.get(membership.user_id) ?? /* @__PURE__ */ new Set();
  ...
  toSearchFilter(value) {
    return (value ?? "").trim().replace(/[%,()]/g, "").slice(0, 80);
  }
  getUserWithMemberships(userId) {
    return __async(this, null, function* () {
      const [userRes, membershipsRes] = yield Promise.all([
        this.supabase.from("users").select("id,email,nome,is_master_admin").eq("id", userId).single(),
        this.supabase.from("user_condominios").select("user_id,condominio_id,role,status,condominio:condominios(nome)").eq("user_id", userId).order("created_at")
      ]);
      if (userRes.error) {
      ...
      return {
        ...
        name: user.nome,
        isMasterAdmin: user.is_master_admin,
        memberships: (membershipsRes.data ?? []).map((membership) => this.mapMembership(membership)).sort((a, b) => a.condominioName.localeCompare(b.condominioName))
      };
    });
  }
  getCondominios() {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("condominios").select("id,nome").is("deleted_at", null).order("nome");
      if (error) {
        throw new Error(error.message);
      }
      return (data ?? []).map((condominio) => ({
        id: condominio.id,
  ...
  updateUser(input2) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("users").update({
        nome: input2.name.trim(),
        is_master_admin: input2.isMasterAdmin
      }).eq("id", input2.id);
      if (error) {
        throw new Error(error.message);
      }
  ...
  addMembership(input2) {
    return __async(this, null, function* () {
      const duplicate = yield this.supabase.from("user_condominios").select("user_id", { head: true, count: "exact" }).eq("user_id", input2.userId).eq("condominio_id", input2.condominioId);
      if (duplicate.error) {
        throw new Error(duplicate.error.message);
      }
      if ((duplicate.count ?? 0) > 0) {
        throw new Error("Este usu\xC3\xA1rio j\xC3\xA1 possui v\xC3\xADnculo com o condom\xC3\xADnio selecionado.");
      }
      const { error } = yield this.supabase.from("user_condominios").insert({
        user_id: input2.userId,
        condominio_id: input2.condominioId,
        role: this.toDbRole(input2.role),
        status: "active"
      });
      if (error) {
        throw new Error(error.message);
      }
      return this.getMembership(input2.userId, input2.condominioId);
    });
  }
  updateMembershipRole(id, role) {
    return __async(this, null, function* () {
      const key = this.parseMembershipId(id);
      const { error } = yield this.supabase.from("user_condominios").update({ role: this.toDbRole(role) }).eq("user_id", key.userId).eq("condominio_id", key.condominioId);
      if (error) {
        throw new Error(error.message);
      }
  ...
  removeMembership(id) {
    return __async(this, null, function* () {
      const key = this.parseMembershipId(id);
      const { error } = yield this.supabase.from("user_condominios").delete().eq("user_id", key.userId).eq("condominio_id", key.condominioId);
      if (error) {
        throw new Error(error.message);
      }
    });
  }
  getMembership(userId, condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("user_condominios").select("user_id,condominio_id,role,status,condominio:condominios(nome)").eq("user_id", userId).eq("condominio_id", condominioId).single();
      if (error) {
        throw new Error(error.message);
      }
  ...
  getCondominioName(condominio, fallback) {
  ...
  parseMembershipId(id) {
    const [userId, condominioId] = id.split(":");
    if (!userId || !condominioId) {
      throw new Error("Identificador de v\xC3\xADnculo inv\xC3\xA1lido.");
    }
...
// libs/core/src/lib/services/platform-dashboard.service.ts
var PlatformDashboardService = class _PlatformDashboardService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
  }
  getDashboard() {
  ...
  static {
    this.\u0275fac = function PlatformDashboardService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _PlatformDashboardService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _PlatformDashboardService, factory: _PlatformDashboardService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(PlatformDashboardService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// libs/core/src/lib/services/tenant-admin.service.ts
var TenantAdminService = class _TenantAdminService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
  }
  list() {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("condominios").select("id,nome,subdomain,email,plan,subscription_status,max_unidades,created_at").is("deleted_at", null).order("created_at", { ascending: false });
      if (error) {
        throw new Error(error.message);
      }
      return data ?? [];
  ...
  create(input2) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("condominios").insert({
        nome: input2.nome,
        cnpj: input2.cnpj,
        subdomain: input2.subdomain,
        email: input2.email,
        plan: input2.plan,
        max_unidades: input2.maxUnidades,
  ...
  updateStatus(id, subscriptionStatus) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("condominios").update({ subscription_status: subscriptionStatus }).eq("id", id);
      if (error) {
        throw new Error(error.message);
      }
    });
  ...
  static {
    this.\u0275fac = function TenantAdminService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _TenantAdminService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _TenantAdminService, factory: _TenantAdminService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(TenantAdminService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// libs/core/src/lib/services/plan-management.service.ts
var PlanManagementService = class _PlanManagementService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
  }
  loadSnapshot() {
    return __async(this, null, function* () {
      const [tenantsRes, featuresRes, planFeaturesRes] = yield Promise.all([
        this.supabase.from("condominios").select("id,nome,plan,subscription_status,trial_ends_at,subscription_expires_at").is("deleted_at", null).order("nome"),
        this.supabase.from("features").select("code,descricao").eq("ativo", true).order("code"),
        this.supabase.from("plano_features").select("plano_nome,feature_code,enabled")
      ]);
      if (tenantsRes.error) {
        throw new Error(tenantsRes.error.message);
      }
      ...
      return {
        ...
        planFeatures: planFeaturesRes.data ?? []
      };
    });
  }
  setFeatureEnabled(plan, featureCode, enabled) {
    return __async(this, null, function* () {
      const { error } = yield this.supabase.from("plano_features").upsert({ plano_nome: plan, feature_code: featureCode, enabled });
      if (error) {
        throw new Error(error.message);
      }
    });
  ...
  static {
    this.\u0275fac = function PlanManagementService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _PlanManagementService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _PlanManagementService, factory: _PlanManagementService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(PlanManagementService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// libs/core/src/lib/services/admin-user.service.ts
var AdminUserService = class _AdminUserService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
  }
  listForTenant(condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("user_condominios").select("role, status, metadata, user:users(id, nome, email)").eq("condominio_id", condominioId).eq("status", "active").order("role");
      if (error) {
        throw new Error(error.message);
      }
      return (data ?? []).map((row) => this.mapRow(row)).filter((row) => row !== null);
    });
  }
  mapRow(row) {
  ...
  static {
    this.\u0275fac = function AdminUserService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _AdminUserService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _AdminUserService, factory: _AdminUserService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AdminUserService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();

// libs/core/src/lib/services/resident.service.ts
var ResidentService = class _ResidentService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.residents = signal([], ...ngDevMode ? [{ debugName: "residents" }] : (
      /* istanbul ignore next */
      []
    ));
    this.isLoading = signal(false, ...ngDevMode ? [{ debugName: "isLoading" }] : (
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("user_condominios").select("user_id,status,metadata,user:users(nome,telefone)").eq("condominio_id", condominioId).eq("role", "morador").order("joined_at", { ascending: false }).limit(100);
      if (error) {
        this.isLoading.set(false);
        this.error.set(error.message);
        this.residents.set([]);
      ...
      const residents = (data ?? []).map(mapResidentRow);
      const enriched = yield Promise.all(residents.map((resident) => __async(this, null, function* () {
        return __spreadProps(__spreadValues({}, resident), {
          pendingDeliveries: yield this.countPendingDeliveries(condominioId, resident.unit)
        });
      })));
  ...
  loadProfile(condominioId, userId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("user_condominios").select("user_id,status,metadata,user:users(nome,telefone)").eq("condominio_id", condominioId).eq("user_id", userId).eq("role", "morador").maybeSingle();
      if (error || !data) {
        return null;
      }
      const resident = mapResidentRow(data);
      const deliveries = yield this.loadDeliveries(condominioId, resident.unit);
      return __spreadProps(__spreadValues({}, resident), {
  ...
  countPendingDeliveries(condominioId, unidade) {
    return __async(this, null, function* () {
      if (!unidade) {
        return 0;
      }
      const { count, error } = yield this.supabase.from("entregas").select("id", { count: "exact", head: true }).eq("condominio_id", condominioId).eq("unidade", unidade).in("status", ["pendente", "notificada"]).is("deleted_at", null);
      return error ? 0 : count ?? 0;
    });
  }
  loadDeliveries(condominioId, unidade) {
    return __async(this, null, function* () {
      if (!unidade) {
        return [];
      }
      const { data, error } = yield this.supabase.from("entregas").select("id,transportadora,tipo,descricao,created_at,status").eq("condominio_id", condominioId).eq("unidade", unidade).is("deleted_at", null).order("created_at", { ascending: false }).limit(20);
      if (error) {
        return [];
      }
      return (data ?? []).map((row) => ({
  ...
  static {
    this.\u0275fac = function ResidentService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _ResidentService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ResidentService, factory: _ResidentService.\u0275fac, providedIn: "root" });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ResidentService, [{
    type: Injectable,
    args: [{ providedIn: "root" }]
  }], null, null);
})();
...
// libs/core/src/lib/features/select-context/select-context.component.ts
var _forTrack0 = ($index, $item) => $item.key;
var _forTrack1 = ($index, $item) => $item.id;
function SelectContextComponent_For_13_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275domElementStart(0, "button", 12);
    \u0275\u0275domListener("click", function SelectContextComponent_For_13_Template_button_click_0_listener() {
      const p_r2 = \u0275\u0275restoreView(_r1).$implicit;
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.selectPlatform(p_r2.key));
    });
  ...
  if (rf & 2) {
    ...
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275styleProp("--card-color", p_r2.color);
    \u0275\u0275classProp("sc__card--active", ctx_r2.selectedPlatform() === p_r2.key);
    \u0275\u0275advance(2);
...
function SelectContextComponent_Conditional_14_For_5_Template(rf, ctx) {
  if (rf & 1) {
    const _r4 = \u0275\u0275getCurrentView();
    \u0275\u0275domElementStart(0, "button", 17);
    \u0275\u0275domListener("click", function SelectContextComponent_Conditional_14_For_5_Template_button_click_0_listener() {
      const t_r5 = \u0275\u0275restoreView(_r4).$implicit;
      const ctx_r2 = \u0275\u0275nextContext(2);
      return \u0275\u0275resetView(ctx_r2.selectTenant(t_r5));
    });
    \u0275\u0275domElementStart(1, "span", 18);
    \u0275\u0275text(2);
    \u0275\u0275domElementEnd();
  ...
  if (rf & 2) {
    ...
    const ctx_r2 = \u0275\u0275nextContext(2);
    \u0275\u0275classProp("sc__tenant--active", ((tmp_11_0 = ctx_r2.selectedTenant()) == null ? null : tmp_11_0.id) === t_r5.id);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(t_r5.nome);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(t_r5.slug);
  }
}
function SelectContextComponent_Conditional_14_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "section", 5)(1, "h2", 6);
    \u0275\u0275text(2, "Condom\xC3\xADnio");
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(3, "div", 15);
    \u0275\u0275repeaterCreate(4, SelectContextComponent_Conditional_14_For_5_Template, 5, 4, "button", 16, _forTrack1);
    \u0275\u0275domElementEnd()();
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275advance(4);
    \u0275\u0275repeater(ctx_r2.tenantsForPlatform());
  }
}
var PLATFORM_CARDS = [
  { key: "morador", label: "Morador", icon: "\xF0\u0178\x8F\xA0", color: "#2d6a4f" },
  { key: "porteiro", label: "Porteiro", icon: "\xF0\u0178\u203A\xA1\xEF\xB8\x8F", color: "#1565c0" },
  { key: "admin", label: "Admin", icon: "\xE2\u0161\u2122\xEF\xB8\x8F", color: "#e65100" },
  { key: "master-admin", label: "Master Admin", icon: "\xE2\xAD\x90", color: "#1a1a2e" }
];
var SelectContextComponent = class _SelectContextComponent {
  constructor() {
    this.ctxSvc = inject(ContextService);
    this.auth = inject(AuthService);
    this.router = inject(Router);
    this.selectedPlatform = signal(null, ...ngDevMode ? [{ debugName: "selectedPlatform" }] : (
      /* istanbul ignore next */
      []
    ));
  ...
  static {
    this.\u0275fac = function SelectContextComponent_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _SelectContextComponent)();
    };
  }
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SelectContextComponent, selectors: [["cm-select-context"]], decls: 20, vars: 2, consts: [[1, "sc"], [1, "sc__header"], [1, "sc__logo"], [1, "sc__title"], [1, "sc__sub"], [1, "sc__section"], [1, "sc__label"], [1, "sc__cards"], [1, "sc__card", 3, "sc__card--active", "--card-color"], [1, "sc__actions"], [1, "sc__confirm", 3, "click", "disabled"], [1, "sc__logout", 3, "click"], [1, "sc__card", 3, "click"], [1, "sc__card-icon"], [1, "sc__card-label"], [1, "sc__tenants"], [1, "sc__tenant", 3, "sc__tenant--active"], [1, "sc__tenant", 3, "click"], [1, "sc__tenant-name"], [1, "sc__tenant-slug"]], template: function SelectContextComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275domElementStart(0, "div", 0)(1, "header", 1)(2, "span", 2);
        \u0275\u0275text(3, "CondoMais");
        \u0275\u0275domElementEnd();
        \u0275\u0275domElementStart(4, "h1", 3);
        \u0275\u0275text(5, "Selecionar contexto");
        \u0275\u0275domElementEnd();
        \u0275\u0275domElementStart(6, "p", 4);
        \u0275\u0275text(7, "Escolha a plataforma e o condom\xC3\xADnio para continuar");
        \u0275\u0275domElementEnd()();
        \u0275\u0275domElementStart(8, "section", 5)(9, "h2", 6);
        \u0275\u0275text(10, "Plataforma");
        \u0275\u0275domElementEnd();
        \u0275\u0275domElementStart(11, "div", 7);
        \u0275\u0275repeaterCreate(12, SelectContextComponent_For_13_Template, 5, 6, "button", 8, _forTrack0);
        \u0275\u0275domElementEnd()();
        \u0275\u0275conditionalCreate(14, SelectContextComponent_Conditional_14_Template, 6, 0, "section", 5);
        \u0275\u0275domElementStart(15, "div", 9)(16, "button", 10);
        \u0275\u0275domListener("click", function SelectContextComponent_Template_button_click_16_listener() {
          return ctx.confirm();
        });
        \u0275\u0275text(17, " Entrar ");
        \u0275\u0275domElementEnd();
        \u0275\u0275domElementStart(18, "button", 11);
        \u0275\u0275domListener("click", function SelectContextComponent_Template_button_click_18_listener() {
          return ctx.logout();
        });
        \u0275\u0275text(19, "Sair");
        \u0275\u0275domElementEnd()()();
      ...
      if (rf & 2) {
        \u0275\u0275advance(12);
        \u0275\u0275repeater(ctx.availablePlatformCards());
        \u0275\u0275advance(2);
        \u0275\u0275conditional(ctx.selectedPlatform() && ctx.tenantsForPlatform().length > 0 ? 14 : -1);
        \u0275\u0275advance(2);
        \u0275\u0275domProperty("disabled", !ctx.canConfirm());
      }
    }, styles: ["\n.sc[_ngcontent-%COMP%] {\n  min-height: 100vh;\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  justify-content: center;\n  gap: 32px;\n  padding: 24px;\n  background: #f4f6f8;\n  box-sizing: border-box;\n}\n.sc__header[_ngcontent-%COMP%] {\n  text-align: center;\n}\n.sc__logo[_ngcontent-%COMP%] {\n  font-size: 13px;\n  font-weight: 700;\n  letter-spacing: 1px;\n  color: #888;\n  text-transform: uppercase;\n}\n.sc__title[_ngcontent-%COMP%] {\n  margin: 8px 0 4px;\n  font-size: 22px;\n  font-weight: 700;\n  color: #1a1a2e;\n}\n.sc__sub[_ngcontent-%COMP%] {\n  margin: 0;\n  font-size: 14px;\n  color: #666;\n}\n.sc__section[_ngcontent-%COMP%] {\n  width: 100%;\n  max-width: 520px;\n}\n.sc__label[_ngcontent-%COMP%] {\n  font-size: 12px;\n  font-weight: 600;\n  color: #888;\n  text-transform: uppercase;\n  letter-spacing: 0.5px;\n  margin: 0 0 10px;\n}\n.sc__cards[_ngcontent-%COMP%] {\n  display: grid;\n  grid-template-columns: repeat(auto-fill, minmax(110px, 1fr));\n  gap: 10px;\n}\n.sc__card[_ngcontent-%COMP%] {\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  gap: 6px;\n  padding: 16px 12px;\n  border-radius: 12px;\n  border: 2px solid #e0e0e0;\n  background: #fff;\n  cursor: pointer;\n  font-size: 13px;\n  font-weight: 500;\n  color: #333;\n  transition: border-color 0.15s, box-shadow 0.15s;\n}\n.sc__card[_ngcontent-%COMP%]:hover {\n  border-color: var(--card-color);\n}\n.sc__card--active[_ngcontent-%COMP%] {\n  border-color: var(--card-color);\n  background: color-mix(in srgb, var(--card-color) 8%, #fff);\n  color: var(--card-color);\n  font-weight: 600;\n  box-shadow: 0 2px 8px color-mix(in srgb, var(--card-color) 25%, transparent);\n}\n.sc__card-icon[_ngcontent-%COMP%] {\n  font-size: 22px;\n}\n.sc__tenants[_ngcontent-%COMP%] {\n  display: flex;\n  flex-direction: column;\n  gap: 8px;\n}\n.sc__tenant[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  padding: 14px 16px;\n  border-radius: 10px;\n  border: 2px solid #e0e0e0;\n  background: #fff;\n  cursor: pointer;\n  text-align: left;\n  transition: border-color 0.15s;\n}\n.sc__tenant[_ngcontent-%COMP%]:hover {\n  border-color: #2d6a4f;\n}\n.sc__tenant--active[_ngcontent-%COMP%] {\n  border-color: #2d6a4f;\n  background: #f0f7f4;\n}\n.sc__tenant-name[_ngcontent-%COMP%] {\n  font-size: 14px;\n  font-weight: 500;\n  color: #1a1a2e;\n}\n.sc__tenant-slug[_ngcontent-%COMP%] {\n  font-size: 12px;\n  color: #999;\n}\n.sc__actions[_ngcontent-%COMP%] {\n  display: flex;\n  gap: 10px;\n  width: 100%;\n  max-width: 520px;\n}\n.sc__confirm[_ngcontent-%COMP%] {\n  flex: 1;\n  padding: 14px;\n  border-radius: 10px;\n  border: none;\n  background: #2d6a4f;\n  color: #fff;\n  font-size: 15px;\n  font-weight: 600;\n  cursor: pointer;\n  transition: opacity 0.15s;\n}\n.sc__confirm[_ngcontent-%COMP%]:disabled {\n  opacity: 0.4;\n  cursor: not-allowed;\n}\n.sc__confirm[_ngcontent-%COMP%]:not(:disabled):hover {\n  opacity: 0.9;\n}\n.sc__logout[_ngcontent-%COMP%] {\n  padding: 14px 20px;\n  border-radius: 10px;\n  border: 1.5px solid #ddd;\n  background: #fff;\n  color: #666;\n  font-size: 14px;\n  cursor: pointer;\n}\n.sc__logout[_ngcontent-%COMP%]:hover {\n  background: #f5f5f5;\n}\n/*# sourceMappingURL=select-context.component.css.map */"], changeDetection: 0 });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SelectContextComponent, [{
    type: Component,
    args: [{ selector: "cm-select-context", standalone: true, changeDetection: ChangeDetectionStrategy.OnPush, template: `
    <div class="sc">
      <header class="sc__header">
        <span class="sc__logo">CondoMais</span>
        <h1 class="sc__title">Selecionar contexto</h1>
        <p class="sc__sub">Escolha a plataforma e o condom\xC3\xADnio para continuar</p>
      </header>
      ...
      @if (selectedPlatform() && tenantsForPlatform().length > 0) {
        <section class="sc__section">
          <h2 class="sc__label">Condom\xC3\xADnio</h2>
  ...
  `, styles: ["/* angular:styles/component:css;3b6cb0a32fbf3905fcc73af7593fae3a8573ef0395cc57cf271fc15a6564785e;C:/Users/Matiasemc/OneDrive/Documentos/git/Condomais/frontend/libs/core/src/lib/features/select-context/select-context.component.ts */\n.sc {\n  min-height: 100vh;\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  justify-content: center;\n  gap: 32px;\n  padding: 24px;\n  background: #f4f6f8;\n  box-sizing: border-box;\n}\n.sc__header {\n  text-align: center;\n}\n.sc__logo {\n  font-size: 13px;\n  font-weight: 700;\n  letter-spacing: 1px;\n  color: #888;\n  text-transform: uppercase;\n}\n.sc__title {\n  margin: 8px 0 4px;\n  font-size: 22px;\n  font-weight: 700;\n  color: #1a1a2e;\n}\n.sc__sub {\n  margin: 0;\n  font-size: 14px;\n  color: #666;\n}\n.sc__section {\n  width: 100%;\n  max-width: 520px;\n}\n.sc__label {\n  font-size: 12px;\n  font-weight: 600;\n  color: #888;\n  text-transform: uppercase;\n  letter-spacing: 0.5px;\n  margin: 0 0 10px;\n}\n.sc__cards {\n  display: grid;\n  grid-template-columns: repeat(auto-fill, minmax(110px, 1fr));\n  gap: 10px;\n}\n.sc__card {\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  gap: 6px;\n  padding: 16px 12px;\n  border-radius: 12px;\n  border: 2px solid #e0e0e0;\n  background: #fff;\n  cursor: pointer;\n  font-size: 13px;\n  font-weight: 500;\n  color: #333;\n  transition: border-color 0.15s, box-shadow 0.15s;\n}\n.sc__card:hover {\n  border-color: var(--card-color);\n}\n.sc__card--active {\n  border-color: var(--card-color);\n  background: color-mix(in srgb, var(--card-color) 8%, #fff);\n  color: var(--card-color);\n  font-weight: 600;\n  box-shadow: 0 2px 8px color-mix(in srgb, var(--card-color) 25%, transparent);\n}\n.sc__card-icon {\n  font-size: 22px;\n}\n.sc__tenants {\n  display: flex;\n  flex-direction: column;\n  gap: 8px;\n}\n.sc__tenant {\n  display: flex;\n  align-items: center;\n  justify-content: space-between;\n  padding: 14px 16px;\n  border-radius: 10px;\n  border: 2px solid #e0e0e0;\n  background: #fff;\n  cursor: pointer;\n  text-align: left;\n  transition: border-color 0.15s;\n}\n.sc__tenant:hover {\n  border-color: #2d6a4f;\n}\n.sc__tenant--active {\n  border-color: #2d6a4f;\n  background: #f0f7f4;\n}\n.sc__tenant-name {\n  font-size: 14px;\n  font-weight: 500;\n  color: #1a1a2e;\n}\n.sc__tenant-slug {\n  font-size: 12px;\n  color: #999;\n}\n.sc__actions {\n  display: flex;\n  gap: 10px;\n  width: 100%;\n  max-width: 520px;\n}\n.sc__confirm {\n  flex: 1;\n  padding: 14px;\n  border-radius: 10px;\n  border: none;\n  background: #2d6a4f;\n  color: #fff;\n  font-size: 15px;\n  font-weight: 600;\n  cursor: pointer;\n  transition: opacity 0.15s;\n}\n.sc__confirm:disabled {\n  opacity: 0.4;\n  cursor: not-allowed;\n}\n.sc__confirm:not(:disabled):hover {\n  opacity: 0.9;\n}\n.sc__logout {\n  padding: 14px 20px;\n  border-radius: 10px;\n  border: 1.5px solid #ddd;\n  background: #fff;\n  color: #666;\n  font-size: 14px;\n  cursor: pointer;\n}\n.sc__logout:hover {\n  background: #f5f5f5;\n}\n/*# sourceMappingURL=select-context.component.css.map */\n"] }]
  }], null, null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SelectContextComponent, { className: "SelectContextComponent", filePath: "libs/core/src/lib/features/select-context/select-context.component.ts", lineNumber: 135 });
})();

// libs/core/src/lib/features/app-header/app-header.component.ts
var _c0 = ["*"];
function AppHeaderComponent_Conditional_8_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(ctx_r0.tenantName());
  }
}
function AppHeaderComponent_Conditional_18_Template(rf, ctx) {
  if (rf & 1) {
    const _r2 = \u0275\u0275getCurrentView();
    \u0275\u0275domElementStart(0, "div", 13)(1, "button", 14);
    \u0275\u0275domListener("click", function AppHeaderComponent_Conditional_18_Template_button_click_1_listener() {
      \u0275\u0275restoreView(_r2);
      const ctx_r0 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r0.switchContext());
    });
    \u0275\u0275namespaceSVG();
    \u0275\u0275domElementStart(2, "svg", 15);
    \u0275\u0275domElement(3, "polyline", 16)(4, "path", 17)(5, "polyline", 18)(6, "path", 19);
    \u0275\u0275domElementEnd();
    \u0275\u0275text(7, " Trocar contexto ");
    \u0275\u0275domElementEnd();
    \u0275\u0275namespaceHTML();
    \u0275\u0275domElement(8, "div", 20);
    \u0275\u0275domElementStart(9, "button", 21);
    \u0275\u0275domListener("click", function AppHeaderComponent_Conditional_18_Template_button_click_9_listener() {
      \u0275\u0275restoreView(_r2);
      const ctx_r0 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r0.logout());
    });
    \u0275\u0275namespaceSVG();
    \u0275\u0275domElementStart(10, "svg", 15);
    \u0275\u0275domElement(11, "path", 22)(12, "polyline", 23)(13, "line", 24);
    \u0275\u0275domElementEnd();
    \u0275\u0275text(14, " Sair ");
    \u0275\u0275domElementEnd()();
  }
...
var AppHeaderComponent = class _AppHeaderComponent {
  constructor() {
    ...
    this.platform = computed(() => this.ctxSvc.getContext()?.platform ?? "morador", ...ngDevMode ? [{ debugName: "platform" }] : (
      /* istanbul ignore next */
      []
    ));
    this.platformLabel = computed(() => PLATFORM_LABELS[this.platform()], ...ngDevMode ? [{ debugName: "platformLabel" }] : (
      /* istanbul ignore next */
      []
    ));
    this.platformColor = computed(() => PLATFORM_COLOR[this.platform()], ...ngDevMode ? [{ debugName: "platformColor" }] : (
      /* istanbul ignore next */
      []
    ));
    this.tenantName = computed(() => this.state.currentTenant()?.nome ?? null, ...ngDevMode ? [{ debugName: "tenantName" }] : (
      /* istanbul ignore next */
      []
    ));
    this.userEmail = computed(() => this.state.profile()?.email ?? this.state.user()?.email ?? "", ...ngDevMode ? [{ debugName: "userEmail" }] : (
      /* istanbul ignore next */
      []
    ));
    this.userInitial = computed(() => (this.userEmail()[0] ?? "?").toUpperCase(), ...ngDevMode ? [{ debugName: "userInitial" }] : (
      /* istanbul ignore next */
      []
    ));
  }
  ...
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AppHeaderComponent, selectors: [["cm-app-header"]], hostBindings: function AppHeaderComponent_HostBindings(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275listener("click", function AppHeaderComponent_click_HostBindingHandler() {
          return ctx.closeMenu();
        }, \u0275\u0275resolveDocument);
      }
    }, ngContentSelectors: _c0, decls: 19, vars: 8, consts: [[1, "ah"], [1, "ah__left"], [1, "ah__logo"], [1, "ah__platform"], [1, "ah__nav"], [1, "ah__tenant"], [1, "ah__right"], [1, "ah__menu"], [1, "ah__trigger", 3, "click"], [1, "ah__initial"], [1, "ah__email"], ["width", "12", "height", "12", "viewBox", "0 0 24 24", "fill", "none", "stroke", "currentColor", "stroke-width", "2.5", "stroke-linecap", "round", "stroke-linejoin", "round", "aria-hidden", "true", 1, "ah__chevron"], ["points", "6 9 12 15 18 9"], [1, "ah__dropdown"], [1, "ah__dd-item", 3, "click"], ["width", "14", "height", "14", "viewBox", "0 0 24 24", "fill", "none", "stroke", "currentColor", "stroke-width", "2", "stroke-linecap", "round", "stroke-linejoin", "round"], ["points", "17 1 21 5 17 9"], ["d", "M3 11V9a4 4 0 0 1 4-4h14"], ["points", "7 23 3 19 7 15"], ["d", "M21 13v2a4 4 0 0 1-4 4H3"], [1, "ah__dd-sep"], [1, "ah__dd-item", "ah__dd-item--danger", 3, "click"], ["d", "M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"], ["points", "16 17 21 12 16 7"], ["x1", "21", "y1", "12", "x2", "9", "y2", "12"]], template: function AppHeaderComponent_Template(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275projectionDef();
        \u0275\u0275domElementStart(0, "header", 0)(1, "div", 1)(2, "span", 2);
        \u0275\u0275text(3, "CondoMais");
        \u0275\u0275domElementEnd();
        \u0275\u0275domElementStart(4, "span", 3);
        \u0275\u0275text(5);
        \u0275\u0275domElementEnd()();
        \u0275\u0275domElementStart(6, "nav", 4);
        \u0275\u0275projection(7);
        \u0275\u0275domElementEnd();
        \u0275\u0275conditionalCreate(8, AppHeaderComponent_Conditional_8_Template, 2, 1, "span", 5);
        \u0275\u0275domElementStart(9, "div", 6)(10, "div", 7)(11, "button", 8);
        \u0275\u0275domListener("click", function AppHeaderComponent_Template_button_click_11_listener($event) {
        ...
        \u0275\u0275domElementEnd();
        \u0275\u0275namespaceSVG();
        \u0275\u0275domElementStart(16, "svg", 11);
        \u0275\u0275domElement(17, "polyline", 12);
        \u0275\u0275domElementEnd()();
        \u0275\u0275conditionalCreate(18, AppHeaderComponent_Conditional_18_Template, 15, 0, "div", 13);
        \u0275\u0275domElementEnd()()();
      }
      if (rf & 2) {
        \u0275\u0275styleProp("background", ctx.platformColor());
        \u0275\u0275advance(5);
        \u0275\u0275textInterpolate(ctx.platformLabel());
        \u0275\u0275advance(3);
        \u0275\u0275conditional(ctx.tenantName() ? 8 : -1);
        \u0275\u0275advance(3);
        ...
        \u0275\u0275advance(3);
        \u0275\u0275conditional(ctx.menuOpen() ? 18 : -1);
      }
    }, styles: ["\n[_nghost-%COMP%] {\n  display: block;\n}\n.ah[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  flex-wrap: nowrap;\n  padding: 0 20px;\n  height: 52px;\n  color: #fff;\n  position: relative;\n  z-index: 100;\n}\n.ah__left[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  flex-shrink: 0;\n}\n.ah__logo[_ngcontent-%COMP%] {\n  font-weight: 700;\n  font-size: 15px;\n  letter-spacing: -0.3px;\n}\n.ah__platform[_ngcontent-%COMP%] {\n  font-size: 10px;\n  font-weight: 600;\n  text-transform: uppercase;\n  letter-spacing: 0.8px;\n  background: rgba(255, 255, 255, 0.18);\n  padding: 2px 7px;\n  border-radius: 999px;\n}\n.ah__nav[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 2px;\n  flex: 1;\n  padding: 0 8px;\n  overflow: hidden;\n}\n[_nghost-%COMP%]     .ah__nav a {\n  color: rgba(255, 255, 255, 0.75);\n  text-decoration: none;\n  font-size: 13px;\n  padding: 5px 10px;\n  border-radius: 6px;\n  transition: background 0.15s, color 0.15s;\n  white-space: nowrap;\n}\n[_nghost-%COMP%]     .ah__nav a:hover {\n  background: rgba(255, 255, 255, 0.12);\n  color: #fff;\n}\n[_nghost-%COMP%]     .ah__nav a.active {\n  background: rgba(255, 255, 255, 0.2);\n  color: #fff;\n  font-weight: 600;\n}\n.ah__tenant[_ngcontent-%COMP%] {\n  font-size: 12px;\n  opacity: 0.7;\n  white-space: nowrap;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  max-width: 160px;\n  flex-shrink: 0;\n}\n.ah__right[_ngcontent-%COMP%] {\n  margin-left: auto;\n  flex-shrink: 0;\n}\n.ah__menu[_ngcontent-%COMP%] {\n  position: relative;\n}\n.ah__trigger[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 6px;\n  background: rgba(255, 255, 255, 0.12);\n  border: 1px solid rgba(255, 255, 255, 0.2);\n  color: #fff;\n  cursor: pointer;\n  padding: 5px 10px;\n  border-radius: 8px;\n  font-size: 13px;\n  transition: background 0.15s;\n}\n.ah__trigger[_ngcontent-%COMP%]:hover {\n  background: rgba(255, 255, 255, 0.2);\n}\n.ah__initial[_ngcontent-%COMP%] {\n  width: 22px;\n  height: 22px;\n  border-radius: 50%;\n  background: rgba(255, 255, 255, 0.25);\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  font-size: 11px;\n  font-weight: 700;\n  flex-shrink: 0;\n}\n.ah__email[_ngcontent-%COMP%] {\n  max-width: 140px;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n.ah__chevron[_ngcontent-%COMP%] {\n  flex-shrink: 0;\n  opacity: 0.7;\n}\n.ah__dropdown[_ngcontent-%COMP%] {\n  position: absolute;\n  right: 0;\n  top: calc(100% + 6px);\n  background: #fff;\n  border-radius: 10px;\n  box-shadow: 0 8px 24px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0, 0, 0, 0.06);\n  min-width: 200px;\n  overflow: hidden;\n  z-index: 200;\n}\n.ah__dd-item[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  width: 100%;\n  padding: 11px 14px;\n  background: none;\n  border: none;\n  text-align: left;\n  cursor: pointer;\n  font-size: 13px;\n  color: #333;\n  transition: background 0.12s;\n}\n.ah__dd-item[_ngcontent-%COMP%]:hover {\n  background: #f5f5f5;\n}\n.ah__dd-item--danger[_ngcontent-%COMP%] {\n  color: #c62828;\n}\n.ah__dd-item--danger[_ngcontent-%COMP%]:hover {\n  background: #fef2f2;\n}\n.ah__dd-sep[_ngcontent-%COMP%] {\n  height: 1px;\n  background: #f0f0f0;\n}\n@media (max-width: 600px) {\n  .ah[_ngcontent-%COMP%] {\n    padding: 0 14px;\n  }\n  .ah__email[_ngcontent-%COMP%] {\n    display: none;\n  }\n  .ah__tenant[_ngcontent-%COMP%] {\n    display: none;\n  }\n  .ah__nav[_ngcontent-%COMP%]   :where(a[_ngcontent-%COMP%]) {\n    font-size: 12px;\n    padding: 4px 7px;\n  }\n}\n/*# sourceMappingURL=app-header.component.css.map */"], changeDetection: 0 });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AppHeaderComponent, [{
    type: Component,
    args: [{ selector: "cm-app-header", standalone: true, changeDetection: ChangeDetectionStrategy.OnPush, template: `
  ...
  `, styles: ["/* angular:styles/component:css;7b3c21e13725d72303185bfcf632bc78726f6a0ca2ef2189fc84b6b244b70aa0;C:/Users/Matiasemc/OneDrive/Documentos/git/Condomais/frontend/libs/core/src/lib/features/app-header/app-header.component.ts */\n:host {\n  display: block;\n}\n.ah {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  flex-wrap: nowrap;\n  padding: 0 20px;\n  height: 52px;\n  color: #fff;\n  position: relative;\n  z-index: 100;\n}\n.ah__left {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  flex-shrink: 0;\n}\n.ah__logo {\n  font-weight: 700;\n  font-size: 15px;\n  letter-spacing: -0.3px;\n}\n.ah__platform {\n  font-size: 10px;\n  font-weight: 600;\n  text-transform: uppercase;\n  letter-spacing: 0.8px;\n  background: rgba(255, 255, 255, 0.18);\n  padding: 2px 7px;\n  border-radius: 999px;\n}\n.ah__nav {\n  display: flex;\n  align-items: center;\n  gap: 2px;\n  flex: 1;\n  padding: 0 8px;\n  overflow: hidden;\n}\n:host ::ng-deep .ah__nav a {\n  color: rgba(255, 255, 255, 0.75);\n  text-decoration: none;\n  font-size: 13px;\n  padding: 5px 10px;\n  border-radius: 6px;\n  transition: background 0.15s, color 0.15s;\n  white-space: nowrap;\n}\n:host ::ng-deep .ah__nav a:hover {\n  background: rgba(255, 255, 255, 0.12);\n  color: #fff;\n}\n:host ::ng-deep .ah__nav a.active {\n  background: rgba(255, 255, 255, 0.2);\n  color: #fff;\n  font-weight: 600;\n}\n.ah__tenant {\n  font-size: 12px;\n  opacity: 0.7;\n  white-space: nowrap;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  max-width: 160px;\n  flex-shrink: 0;\n}\n.ah__right {\n  margin-left: auto;\n  flex-shrink: 0;\n}\n.ah__menu {\n  position: relative;\n}\n.ah__trigger {\n  display: flex;\n  align-items: center;\n  gap: 6px;\n  background: rgba(255, 255, 255, 0.12);\n  border: 1px solid rgba(255, 255, 255, 0.2);\n  color: #fff;\n  cursor: pointer;\n  padding: 5px 10px;\n  border-radius: 8px;\n  font-size: 13px;\n  transition: background 0.15s;\n}\n.ah__trigger:hover {\n  background: rgba(255, 255, 255, 0.2);\n}\n.ah__initial {\n  width: 22px;\n  height: 22px;\n  border-radius: 50%;\n  background: rgba(255, 255, 255, 0.25);\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  font-size: 11px;\n  font-weight: 700;\n  flex-shrink: 0;\n}\n.ah__email {\n  max-width: 140px;\n  overflow: hidden;\n  text-overflow: ellipsis;\n  white-space: nowrap;\n}\n.ah__chevron {\n  flex-shrink: 0;\n  opacity: 0.7;\n}\n.ah__dropdown {\n  position: absolute;\n  right: 0;\n  top: calc(100% + 6px);\n  background: #fff;\n  border-radius: 10px;\n  box-shadow: 0 8px 24px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0, 0, 0, 0.06);\n  min-width: 200px;\n  overflow: hidden;\n  z-index: 200;\n}\n.ah__dd-item {\n  display: flex;\n  align-items: center;\n  gap: 8px;\n  width: 100%;\n  padding: 11px 14px;\n  background: none;\n  border: none;\n  text-align: left;\n  cursor: pointer;\n  font-size: 13px;\n  color: #333;\n  transition: background 0.12s;\n}\n.ah__dd-item:hover {\n  background: #f5f5f5;\n}\n.ah__dd-item--danger {\n  color: #c62828;\n}\n.ah__dd-item--danger:hover {\n  background: #fef2f2;\n}\n.ah__dd-sep {\n  height: 1px;\n  background: #f0f0f0;\n}\n@media (max-width: 600px) {\n  .ah {\n    padding: 0 14px;\n  }\n  .ah__email {\n    display: none;\n  }\n  .ah__tenant {\n    display: none;\n  }\n  .ah__nav :where(a) {\n    font-size: 12px;\n    padding: 4px 7px;\n  }\n}\n/*# sourceMappingURL=app-header.component.css.map */\n"] }]
  }], null, { closeMenu: [{
    type: HostListener,
    args: ["document:click"]
  }] });
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AppHeaderComponent, { className: "AppHeaderComponent", filePath: "libs/core/src/lib/features/app-header/app-header.component.ts", lineNumber: 154 });
})();
...
// libs/core/src/lib/guards/context.guard.ts
function contextGuard(appKey) {
  return () => {
    const state = inject(AuthState);
...
// libs/core/src/lib/auth/role.guard.ts
...
// libs/core/src/lib/providers/core.providers.ts
function provideCore(config) {
  return makeEnvironmentProviders([
    ...
    { provide: AUTH_APP_KEY, useValue: config.appKey },
    {
      ...
      useFactory: (cfg) => supabaseClientFactory(cfg),
      deps: [SUPABASE_CONFIG]
    },
    {
      provide: APP_INITIALIZER,
      useFactory: (authSvc) => () => authSvc.init(),
      deps: [AuthService],
      multi: true
    }
  ]);
...
//# sourceMappingURL=chunk-IQFUSNZ4.js.map
