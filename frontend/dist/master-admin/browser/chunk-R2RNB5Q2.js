...
// libs/core/src/lib/supabase/client.ts
...
var AuthState = class _AuthState {
  constructor() {
    ...
    this.memberships = signal([]);
    this.currentTenant = signal(null);
    ...
    this.activeMemberships = computed(() => this.memberships().filter((m) => m.ativo));
    this.hasMultipleTenants = computed(() => this.activeMemberships().length > 1);
    this.isMasterAdmin = computed(() => this.memberships().some((m) => m.ativo && (m.role === "MASTER_ADMIN" || m.role === "master_admin")));
  }
  static {
...
var MembershipService = class _MembershipService {
  ...
  load(userId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("user_condominios").select("id, role, ativo, metadata, condominio:condominios(id, nome, slug, plano, ativo)").eq("user_id", userId).eq("ativo", true);
      if (error) {
  ...
  login(email, password) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        yield this.memberships.load(data.user.id);
...
var UserManagementService = class _UserManagementService {
  ...
  getUsers() {
    return __async(this, null, function* () {
      const [usersRes, membershipsRes] = yield Promise.all([
        this.supabase.from("users").select("id,email,nome,is_master_admin,created_at").order("nome"),
        this.supabase.from("user_condominios").select("user_id,condominio_id,status").eq("status", "active")
      ]);
      if (usersRes.error) {
  ...
  addMembership(input) {
    return __async(this, null, function* () {
      const duplicate = yield this.supabase.from("user_condominios").select("user_id", { head: true, count: "exact" }).eq("user_id", input.userId).eq("condominio_id", input.condominioId);
      if (duplicate.error) {
        throw new Error(duplicate.error.message);
      }
      if ((duplicate.count ?? 0) > 0) {
        throw new Error("Este usu\xE1rio j\xE1 possui v\xEDnculo com o condom\xEDnio selecionado.");
      }
  ...
  parseMembershipId(id) {
    ...
    if (!userId || !condominioId) {
      throw new Error("Identificador de v\xEDnculo inv\xE1lido.");
    }
...
var DeliveryService = class _DeliveryService {
  constructor() {
    ...
    this.error = signal(null);
    this.channel = null;
    this.realtimeEvents$ = new Subject();
  }
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      ...
      this.error.set(null);
      const { data, error } = yield this.supabase.from("entregas").select("*").eq("condominio_id", condominioId).is("deleted_at", null).order("created_at", { ascending: false });
  ...
  loadForUnit(condominioId, unidade) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("entregas").select("*").eq("condominio_id", condominioId).eq("unidade", unidade).is("deleted_at", null).order("created_at", { ascending: false });
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadById(id) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("entregas").select("*").eq("id", id).is("deleted_at", null).single();
      if (error || !data)
        return null;
      return mapRow(data);
    });
  ...
  loadForUser(userId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("entregas").select("*").eq("morador_id", userId).is("deleted_at", null).order("created_at", { ascending: false });
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
...
var OccurrenceService = class _OccurrenceService {
  ...
  loadMyOccurrences(condominioId, userId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("ocorrencias").select("*").eq("condominio_id", condominioId).eq("user_id", userId).order("created_at", { ascending: false });
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("ocorrencias").select("*").eq("condominio_id", condominioId).order("created_at", { ascending: false });
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadById(id) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("ocorrencias").select("*").eq("id", id).single();
      if (error || !data)
        return null;
      return mapRow2(data);
    });
  ...
  loadImages(ocorrenciaId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("ocorrencia_imagens").select("*").eq("ocorrencia_id", ocorrenciaId).order("ordem");
      if (error || !data)
        return [];
  ...
  create(input) {
    return __async(this, null, function* () {
      ...
      if (error || !data) {
        this.error.set(error?.message ?? "Erro ao criar ocorr\xEAncia");
        return null;
      }
...
var NotificationService = class _NotificationService {
  constructor() {
    ...
    this.sub.add(this.deliveryService.realtimeEvents$.subscribe((event) => {
    ...
    this.sub.add(this.occurrenceService.realtimeEvents$.subscribe((event) => {
      const o = event.occurrence;
      if (event.eventType === "INSERT") {
        this.push({
          id: crypto.randomUUID(),
          type: "nova_ocorrencia",
          title: "Nova ocorr\xEAncia",
        ...
        if (statusLabel) {
          this.push({
            ...
            title: "Ocorr\xEAncia atualizada",
...
var ReservationService = class _ReservationService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.reservations = signal([]);
    this.equipamentos = signal([]);
    this.isLoading = signal(false);
    this.error = signal(null);
    this.channel = null;
    this.realtimeEvents$ = new Subject();
  ...
  loadEquipamentos(condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("equipamentos").select("*").eq("condominio_id", condominioId).eq("ativo", true).order("nome");
      if (error) {
        this.error.set(error.message);
        return;
      }
  ...
  loadAllEquipamentos(condominioId) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("equipamentos").select("*").eq("condominio_id", condominioId).order("nome");
      if (error) {
        this.error.set(error.message);
        return;
      }
  ...
  loadMyReservations(condominioId, userId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("reservas").select("*, equipamento:equipamentos(id,nome,image_url)").eq("condominio_id", condominioId).eq("morador_id", userId).is("deleted_at", null).order("data", { ascending: false }).order("hora_inicio", { ascending: false });
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      this.error.set(null);
      const { data, error } = yield this.supabase.from("reservas").select("*, equipamento:equipamentos(id,nome,image_url), morador:users(id,nome,email)").eq("condominio_id", condominioId).is("deleted_at", null).order("data", { ascending: false }).order("hora_inicio", { ascending: false });
      this.isLoading.set(false);
      if (error) {
        this.error.set(error.message);
        return;
  ...
  loadForDate(equipamentoId, data) {
    return __async(this, null, function* () {
      const { data: rows, error } = yield this.supabase.from("reservas").select("*").eq("equipamento_id", equipamentoId).eq("data", data).eq("status", "confirmada").is("deleted_at", null);
      if (error || !rows)
        return [];
  ...
  create(input) {
    return __async(this, null, function* () {
      const { data, error } = yield this.supabase.from("reservas").insert({
        ...
        status: "confirmada"
      }).select("*, equipamento:equipamentos(id,nome,image_url)").single();
      if (error || !data) {
...
var BillingService = class _BillingService {
  constructor() {
    this.supabase = inject(SUPABASE_CLIENT);
    this.subscription = signal(null);
    this.planos = signal([]);
    this.isLoading = signal(false);
    this.planNome = computed(() => this.subscription()?.plano_nome ?? "free");
    this.isActive = computed(() => {
      const s = this.subscription()?.status;
      return s === "ACTIVE" || s === "TRIALING";
    });
    this.isPastDue = computed(() => this.subscription()?.status === "PAST_DUE");
    this.canAccessOccurrences = computed(() => this.isActive() && ["basic", "plus", "premium"].includes(this.planNome()));
    this.canAccessReservations = computed(() => this.isActive() && ["plus", "premium"].includes(this.planNome()));
    this.canAccessMarketplace = computed(() => this.isActive() && this.planNome() === "premium");
  }
  loadForTenant(condominioId) {
    return __async(this, null, function* () {
      this.isLoading.set(true);
      const [subRes, planosRes] = yield Promise.all([
        this.supabase.from("subscriptions").select("id,condominio_id,plano_nome,status,current_period_end,stripe_customer_id").eq("condominio_id", condominioId).maybeSingle(),
        this.supabase.from("planos").select("nome,label,price_monthly_brl,features").order("price_monthly_brl")
      ]);
      this.subscription.set(subRes.data ?? null);
      this.planos.set(planosRes.data ?? []);
      this.isLoading.set(false);
    });
  }
  ...
  getCurrentSubscription(condominioId) {
    return __async(this, null, function* () {
      const { data } = yield this.supabase.from("subscriptions").select("id,condominio_id,plano_nome,status,current_period_end,stripe_customer_id").eq("condominio_id", condominioId).maybeSingle();
      const sub = data ?? null;
...
// libs/core/src/lib/guards/master-admin.guard.ts
var masterAdminGuard = () => {
...
//# sourceMappingURL=chunk-R2RNB5Q2.js.map
